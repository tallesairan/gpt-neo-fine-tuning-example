...
RUN apt -y update
RUN apt install -y libaio-dev wget curl htop atop bash zip unzip
...
COPY ./train "/app"
RUN cd /app
RUN wget "https://pnwrld.porno16.com/dataset.csv.zip"
RUN unzip dataset.csv.zip
ARG REF=main
...
RUN python gpt_neo_xl_deepspeed.py